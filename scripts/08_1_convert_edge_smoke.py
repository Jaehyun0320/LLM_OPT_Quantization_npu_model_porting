...
def parse_args():
    ...
    parser.add_argument("--model-id", type=str, default="google/gemma-3-270m")
    parser.add_argument("--edge-format", type=str, default="mlx", choices=["mlx"])
    parser.add_argument("--mlx-path", type=str, default="models/gemma3_270m_mlx_4bit")
    ...
    parser.add_argument("--convert-timeout-sec", type=int, default=3600)
    parser.add_argument("--generate-timeout-sec", type=int, default=600)
    parser.add_argument("--output", type=str, default="results/edge_mlx_smoke_gemma3.json")
...
def main():
    ...
    result = {
        ...
        "purpose": (
            "Smoke test for the Apple MLX conversion toolchain using a smaller "
            "Gemma 3 model supported by mlx-lm. This is not a replacement for "
            "the required Gemma 4 E2B conversion attempt."
        ),
    ...
    result["convert"] = run_command(
        command=build_convert_command(args),
    ...
    if not result["success"]:
        result["notes"].append(
            "This smoke test failed. Inspect convert.stderr and generate.stderr "
            "to identify whether the issue is model support, authentication, "
            "download, local memory, or the MLX/Metal runtime."
        )
    ...
    print(f"Wrote MLX smoke result to {args.output}")
